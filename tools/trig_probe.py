...
static void arm_model(float input, float *co, float *si) {
  double x=fabs((double)input);
  int n=(int)floor(x*0x1.45F306DC9C883p-1+0.5);
...
''')
    ...
    report = dict(passed=False,directions=count,limit=limit,profile='GnuGradient' if gnu else 'AccurateGradient',reference='arm-model' if args.gnu_control else 'native-libm',source_sha256=source_gate(),lanes={})
    for lane in lanes:
...
def values(n: Nat, +direction: F32) -> List<(U32 & U32)>:
  match n:
    case 0n: Nil{}
    case 1n+k:
      Con{bits(T.sincos_for(GNU_PROFILE, ((90.0 - direction) / 180.0 * 3.14159 : F32))), values(k, (direction + 1.0 : F32))}
...
'''.replace('GNU_PROFILE','True{}' if gnu else 'False{}').replace('BANG','!' if lane=='metal' else '').replace('BATCHES',str(count//32))
