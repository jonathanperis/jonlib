...
- Core `raylib.h`: **600 functions**, **98 partial**, **0 complete**.
...
| 7 | 1 | `pixels` — Colors and pixel formats | 46 | 16 | 0 | numerics, memory |
| 8 | 1 | `images` — Image creation, primitives and transformations | 54 | 50 | 0 | pixels, memory |
...
| 16 | 2 | `shapes` — 2D drawing and splines | 62 | 4 | 0 | frame, rlgl-core, raymath, pixels |
...
- **`raylib:function:GetPixelColor`** (not-started, `pixels`): Derive GetPixelColor semantics from the linked source, add a reference fixture, then implement Color.get_pixel_color.
- **`raylib:function:SetPixelColor`** (not-started, `pixels`): Derive SetPixelColor semantics from the linked source, add a reference fixture, then implement Color.set_pixel_color.
- **`raylib:function:ImageFormat`** (not-started, `pixels`): Derive ImageFormat semantics from the linked source, add a reference fixture, then implement Color.image_format.
