...
- Core `raylib.h`: **600 functions**, **87 partial**, **0 complete**.
...
| 8 | 1 | `images` — Image creation, primitives and transformations | 54 | 45 | 0 | pixels, memory |
...
- **`raylib:function:GenImagePerlinNoise`** (not-started, `images`): Derive GenImagePerlinNoise semantics from the linked source, add a reference fixture, then implement Surface.gen_image_perlin_noise.
- **`raylib:function:GenImageText`** (not-started, `fonts`): Derive GenImageText semantics from the linked source, add a reference fixture, then implement Font.gen_image_text.
- **`raylib:function:ImageColorGrayscale`** (not-started, `images`): Derive ImageColorGrayscale semantics from the linked source, add a reference fixture, then implement Surface.image_color_grayscale.
- **`raylib:function:LoadImagePalette`** (not-started, `images`): Derive LoadImagePalette semantics from the linked source, add a reference fixture, then implement Surface.load_image_palette.
- **`raylib:function:UnloadImagePalette`** (not-started, `images`): Derive UnloadImagePalette semantics from the linked source, add a reference fixture, then implement Surface.unload_image_palette.
