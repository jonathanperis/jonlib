...
The current local Apple M1/macOS corpus contains **157 deterministic scenarios /
32,599 checked output words per lane**, matching native CPU (one and two threads), JavaScript
and forced Metal. Most words are RGBA pixels; 172 are exact scalar/Vector2/collision
...
`.build/api-inventory.json` is its legacy core view, mapping 66 reference APIs to
...
| Rectangle/circle queries | `Collision.recs/circles/rectangle` | Exact Booleans and overlap rectangle bits, with reference boundary/degenerate rules |
